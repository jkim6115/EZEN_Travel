...
public class BoardController {
	...
	public ModelAndView list(PageDTO pv, ModelAndView mav, HttpSession httpSession, @ModelAttribute GeneralDTO gdto) {
		// System.out.println(httpSession.getAttribute("user_num"));
		// System.out.println(httpSession.getAttribute("nickname"));
		...
		String user_num = (String) httpSession.getAttribute("user_num");
		String nickname = (String) httpSession.getAttribute("nickname");
		gdto.setUser_num(user_num);
		gdto.setNickname(nickname);
		mav.addObject("gdto", gdto);
		// mav.addObject("userNum", httpSession.getAttribute(user));
		// mav.addObject("nickname", httpSession.getAttribute(nickname));
		// borad로 뷰 설정
		mav.setViewName("board/board");
		return mav;
	} // end list()
	...
	public ModelAndView write(Cm_boardDTO dto, PageDTO pv, ModelAndView mav, HttpSession httpSession,
			@ModelAttribute GeneralDTO gdto) {
	...
	public ModelAndView view(int currentPage, int num, ModelAndView mav, HttpSession httpSession,
			@ModelAttribute GeneralDTO gdto) {
	...
	public ModelAndView update(Cm_boardDTO dto, int currentPage, ModelAndView mav, HttpSession httpSession,
			@ModelAttribute GeneralDTO gdto) {
	...
	public ModelAndView delete(Cm_boardDTO dto, int currentPage, PageDTO pv, ModelAndView mav, HttpSession httpSession,
			@ModelAttribute GeneralDTO gdto) {
	...
	public String commentWrite(Cm_commentDTO dto, int currentPage, HttpSession httpSession,
			@ModelAttribute GeneralDTO gdto) {
		...
		if(gdto.getUser_num() == null) {
			return "redirect:/mainmap.do";
		}
		// 댓글을 작성한 게시글로 뷰 설정
		return "redirect:/view.do?currentPage=" + currentPage + "&num=" + num;
	} // end commentWrite()
	...
	public ModelAndView commentUpdate(Cm_commentDTO dto, int currentPage, ModelAndView mav, HttpSession httpSession,
			@ModelAttribute GeneralDTO gdto) {
	...
	public String commentDelete(Cm_commentDTO dto, int currentPage, HttpSession httpSession,
			@ModelAttribute GeneralDTO gdto) {
