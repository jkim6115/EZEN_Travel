...
var mcOptions = {//마커 클러스터 옵션(아이콘 설정 등)
  styles: [
    {
      height: 40,
      width: 40,
...
function initMap() {
  ...
  document.querySelector('.logo').style.left = -104 + 'px';
  $.ajax({
    type: 'POST',
    dataType: 'json',
    url: 'mainmap.do',
    success: function (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        loc.push({
          ...
          area_image2: data[i].area_image2,
      ...
      function showMarkersInViewport() {
        ...
        function selectDataRow(area_title, idx) {
          ...
          for (var i = 0; i < cListUl.length; i++) { //이미 매칭된 리스트 데이터가 있다면 테두리를 초기화한다.
