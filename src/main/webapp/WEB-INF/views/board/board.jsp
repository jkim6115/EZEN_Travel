				...
				<div class="board_btn">
					...
					<form id="frm_wr" name="frm_wr" method="get" action="write.do">
						<input type="button" id="write" value="글쓰기" /> <input
							type="hidden" id="user_num" value="${gdto.user_num}" />
					</form>
				</div>
				<!-- 게시판 리스트 출력 -->
					...
					<tbody id="board_info">
						...
						<c:forEach items="${gList}" var="dto">
							<tr>
								<td>${dto.bno}</td>
								<td>
									<%-- 게시판 넘어가는 url 밸류값 --%> <%-- 현재 페이지, 게시판 번호, 검색 키워드, 검색 단어 --%>
									...
									</c:url> <%-- 제목 클릭시 path값 같이 보내면서 페이지 이동 --%> <a href="${path}">${dto.title}</a>
								</td>
				...
				<div class="board_list">
					<div class="board_list_box">
						...
						<c:if test="${pv.startPage>1}">
							<a
								href="board.do?currentPage=${pv.startPage-pv.blockPage}${pv.searchInfo}"
								class="prev"> 이전 </a>
						</c:if>
						<!-- 이전 출력 끝 -->
						...
						<c:forEach var="i" begin="${pv.startPage}" end="${pv.endPage}">
							<span id="page_list"> <c:url var="currPage"
									value="board.do">
						...
						<div class="board_search_wrap">
							...
							<div class="board_search_box">
								...
								<div class="inner_search">
									<input class="inner_keyword" type="text" name="searchWord"
										title="검색어 입력" />
								</div>
